...
CCGO Imager MCP 可以让 Codex / Claude 在本机通过你的 CCGO 生图 MCP Key 生成或编辑图片。安装完成后，你不需要记工具名，直接用自然语言描述想要的图片即可。
...
不支持 MCP 的普通 API 客户端、聊天软件或网页模型页面，不能直接使用这个工具。
...
```text
1. 下载并解压 CCGO-Imager-MCP-mac 压缩包
2. 双击 CCGO-Imager-Installer.command
3. 在打开的终端窗口里粘贴 CCGO 生图 MCP Key
4. 看到“安装完成”后，重启 Codex / Claude
```

不要把真实 Key 发到聊天窗口、公开仓库或截图里。安装器会在终端里隐藏输入，并只显示脱敏后的 Key 给你确认。

## 怎么使用

重启 Codex / Claude 后，直接说你想要什么：
...
如果请求的是 4K 或其他耗时图片，MCP 会自动等待生成完成并保存结果。成功后会立即返回，不会固定等待 15 分钟；15 分钟只是默认最长等待时间。

## Key 和安全

- Key 会保存在你本机的 Codex / Claude 配置文件里，用于之后自动调用。
- 安装器会在写配置前自动备份原文件，并尽量收紧配置文件权限。
- 不要把 Key、配置文件截图、终端截图发到聊天窗口或公开地方。
...
- 批量编辑默认并发 4，最大并发 6。
- 单张输入图片默认不超过 30MB。
- 支持读取 PNG / JPEG / WebP / GIF 输入图片。
- 输出文件只会写入 `~/Pictures/ccgo-imager-output` 或你配置的输出目录。

## 卸载或更换 Key

移除 Codex / Claude 里的 MCP 配置：
...
普通用户不需要手动执行这些命令。需要排障或自定义路径时再看这一节。
...
- `n` 限制为 1-6。
- 批量编辑默认并发 4，最大并发 6。
- 大图异步任务默认最长等待 15 分钟，成功后立即返回，不会固定等满。
- 单张输入图默认限制 30MB。
- 输入图片会校验 PNG/JPEG/WebP/GIF 文件头，不只看扩展名。
- 输出路径只能写入 `CCGO_IMAGER_OUTPUT_DIR` 内部，避免路径穿越。
- URL 图片下载只用于保存 CCGO 返回的图片结果 URL；只允许 `http/https` 公网地址，拒绝 localhost、内网、链路本地、保留地址等风险目标，并会对重定向目标重新校验。
